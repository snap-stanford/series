import sys, os
from lanechanging_detection import * 

dict_sessions = dict()

# extract lane chaging from filenames given the start_lines and end_lines of turns
# where no line overlapping exists.
...
def lane_changing_extractor(filename, start_line, end_line):
...
# in each sublist, 
# so that we can read the file for a couple of times to get all lane changings out.
def lane_changing_extractor_all(filename, start_line, end_line):
    starts = [[], []]
    ends = [[], []]
    ...
    for i in range(0, len(starts)):
        lane_changing_extractor(filename, starts[i], ends[i])


# output a lane changing
def output(sid, heading, lat, lng, lines):
    heading_before = avg_direction(heading[:10])
    heading_after = avg_direction(heading[-10:])
    center = len(lat) / 2
    clat = lat[center]
    clng = lng[center]
    suffix = '_%.6f_%.6f_%03d_%03d' % (clat, clng, heading_before, heading_after)
    dir = '/dfs/scratch0/silei/vw_project/lane_changings_freeway2/'
...
def extract_lane_changings(filename):
    get_session_dict()
    dir = '/dfs/scratch0/silei/vw_project/snapshots/'
    snapshot = os.path.join(dir, filename)
    starts, ends = detect_lane_changing(snapshot)
    lane_changing_extractor_all(filename, starts, ends)


if __name__ == '__main__':
    extract_lane_changings(sys.argv[1])
